...
define ln (source, dest, opts)
{
  ...
  if (NULL == st_source)
    ...
    if (NULL == st_source)
      {
      (@print_err) (sprintf ("accessing `%s': No such file or directory", source));
      return -1;
      }
    }
  ...
  if ((source == dest)
     ...
     && (st_source.st_dev == st_dest.st_dev &&
       NULL == opts.nodereference && NULL == opts.force)))
    {
    (@print_err) (sprintf ("`%s' and `%s' are the same file", source, dest));
    return -1;
    }

  if (NULL == opts.symbolic && stat_is ("dir", st_source.st_mode))
    {
    (@print_err) (sprintf ("`%s': hard link not allowed for directory", dest));
    return -1;
    }
  ...
  if (NULL != st_dest && stat_is ("dir", st_dest.st_mode) && NULL == opts.nodereference)
    {
    (@print_err) (sprintf ("`%s': cannot overwrite directory", source));
    return -1;
    }
  ...
  if (NULL != st_dest)
    ...
    if (opts.backup || opts.force)
      ...
      if (stat_is ("lnk", st_dest.st_mode))
        ...
        if (-1 == symlink (value, backupdest))
          {
          (@print_err) (sprintf ("creating backup symbolic link failed `%s', ERRNO: %s",
        ...
        {
        (@print_err) (
            "Operation is not permitted, dest is not neither a link or a regular file");
        return -1;
        }
    ...
    if (opts.force)
      ...
      if (-1 == retval)
        {
        (@print_err) (sprintf ("%s: destination cannot be removed", dest));
        return -1;
        }
      }
    }
      ...
      if (stat_is ("dir", tmp.st_mode))
        if (opts.nodereference && (opts.force || opts.backup))
          if (-1 == remove (dest))
            ...
            (@print_err) (sprintf ("%s: cannot be removed", dest));
            return -1;
            }
    }
