define copyfile (source, dest)
{
  ...
  if (NULL == source_fp)
    {
    (@print_err) (sprintf ("Unable to open: `%s': %s", source, errno_string (errno)));
    return -1;
    }
  ...
  if (NULL == dest_fp)
    {
    (@print_err) (sprintf ("Unable to open: `%s': %s", dest, errno_string (errno)));
    return -1;
    }
