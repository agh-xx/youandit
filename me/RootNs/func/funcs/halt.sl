define main (self)
{
  ...
  ifnot (qualifier_exists ("force"))
    {
    retval = root.lib.ask (["do you want to halt the machine?", "[y/n/escape to abort]"],
      ['y', 'n'];header = "QUESTION FROM THE HALT FUNCTION");
  ...
  if ((0 == pid) && -1 == execv (SUDO_EXEC, ["sudo", "-S", "/sbin/shutdown", "-h", "now"]))
    {
    srv->send_msg ("Could't halt the machine", -1);
    throw GotoPrompt;
    }
