...
define main ()
{
  ...
  ifnot (NULL == tf)
    ...
    if (6 != length (tok))
      {
      (@print_err) ("Error while parsing the time format");
      return 1;
      }
  ...
  _for i (0, length (files) - 1)
    ...
    if (NULL == st)
      ...
      ifnot (NULL == nocreate)
        {
        (@print_err) (sprintf ("`%s': No such file and --no-create is given", files[i]));
        exit_code = 1;
        continue;
        }
      ...
      if (NULL == fp)
        {
        (@print_err) (sprintf ("cannot touch `%s', ERRNO: %s", files[i], errno_string (errno)));
        exit_code = 1;
        continue;
        }
