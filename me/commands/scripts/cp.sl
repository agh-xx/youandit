...
define main ()
{
  ...
  if (opts.noclobber && opts.backup)
    {
    (@print_err) ("Options: `--backup' and `--no-clobber' are mutually exclusive");
    return 1;
    }
  ...
  if ((NULL == stat_dest || 0 == stat_is ("dir", stat_dest.st_mode))
    && 1 < length (files))
    {
    (@print_err) (sprintf ("target %s is not a directory", dest));
    return 1;
    }
  ...
  _for i (0, length (files) -1)
    ...
    if (NULL == st_source)
      {
      (@print_err) (sprintf ("cannot stat `%s': No such file or directory", source));
      exit_code = 1;
      continue;
      }
    ...
    if (isdir_source)
      if (NULL == recursive)
        ...
        {
        if (-1 == cprecursive (source, destname, opts))
          exit_code = 1;
        continue;
        }
