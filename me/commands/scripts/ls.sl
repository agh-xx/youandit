...
define assign_filetype (type, filetype, code)
{
  ...
  _for i(0,length (ar)-1)
    ...
    if (NULL == index)
      {
      (@print_err) (sprintf ("%s: Wrong file type, valid are: %s", ar[i],
            strjoin (st_mode, " - ")));
      exit (1);
      }
...
define main ()
{
  variable
    opts = struct
      {
      executablesonly, %bad programming, make it an option to match `mode bits', but cheap
      find,
      ignore,
      match,
      sort_func_string,
