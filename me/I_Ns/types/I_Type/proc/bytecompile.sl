...
variable
  ...
  EXCLUDEDIRSFORDELETION = [sprintf ("%s/tmp", ROOTDIR), sprintf ("%s/modules", STDNS)],
...
define file_callback_a (file, st)
{
  ...
  variable newfile = strreplace (file, ROOTDIR, SOURCEDIR);
...
define dir_callback_a (dir, st, dirs)
{
  ...
  variable newdir = strreplace (dir, ROOTDIR, SOURCEDIR);
...
define main ()
{
  if (-1 == chdir (SOURCEDIR))
    {
    (@print_err)  (sprintf ("cannot change to sources directory ERRNO: %s",
      errno_string (errno)));
    return 1;
    }
