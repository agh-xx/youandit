...
define main (self, name)
{
  ...
  if (-1 == access (sprintf ("%s/data/repos.txt", mytypedir), F_OK))
    {
    fp = fopen (sprintf ("%s/data/repos.txt", mytypedir), "w");
    if (NULL == fp)
      {
    ...
    repos = readfile (sprintf ("%s/data/repos.txt", mytypedir));
  ...
  root.windows[name] = struct
    ...
    addframe = &addframe,
    git = git,
    reposfile = sprintf ("%s/data/repos.txt", mytypedir),
    repos = repos,
    cur = struct
      {
