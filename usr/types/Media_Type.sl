...
define main (self, name)
{
  ...
  root.windows[name] = struct
    {
    @root.windows[name],
    datadir = sprintf ("%s/../%s", DATADIR, path_basename_sans_extname  (__FILE__)),
    ...
    quit = &quit,
    conf,
    fifo = sprintf ("%s/%s/Constant_mplayer_fifo", TEMPDIR, name),
    fd_fifo,
    playlistfile = sprintf ("%s/%s/playlist.txt", TEMPDIR, name),
    lyricsdir = sprintf ("%s/data/lyrics", ROOTDIR),
    todolyricfile = sprintf ("%s/Todo_Type/lyrics", DATADIR),
    outputfile = sprintf ("%s/%s/output.txt", TEMPDIR, name),
    pat = "^Playing.*\\056$",
    mediaext = [".mp3", ".ogg", ".mp4", ".mpg", ".avi", ".m4v", ".mkv", ".mp4", ".mov", ".flv", ".AVI", ".flac"],
  ...
  me.conf = sprintf ("%s/input.conf", me.datadir);
  me.minframes = 2;
  me.frame_size = 5;
