...
private define exec (file)
{
  ...
  while (i < length (ar))
    ...
    try
      ...
      {
      (@print_err) (sprintf ("caught an error in exec func in script: %s", __FILE__));
      (@print_err) (sprintf ("file that occured: %s", file));
      (@print_err) (sprintf ("linenr that occured: %d", i));
...
private define grep (file, depth)
{
  ...
  if (-1 == access (file, F_OK|R_OK))
    {
    (@print_err) (sprintf ("%s: %s", file, errno_string (errno));print_in_msg_line);
    return 1;
    }
...
define main ()
{
  ...
  if (NULL == PAT && NULL == danglinglinks)
    {
    (@print_err) (sprintf (
      "%s: pattern was not given, I don't know what to look", __argv[0]);print_in_msg_line);
    return 1;
    }
